...
# phần kiểm tra tên
    ...
    # Thêm vào class ALSRec trong rc_CF.py
    def recommend_from_likes(self, selected_movie_ids, top_k=10):
        """
        Tạo gợi ý cho người dùng mới dựa trên danh sách movieId họ vừa chọn
        """
        import numpy as np
        ...
        for mid in selected_movie_ids:
            if mid in self.itemmap:
                user_items_indices.append(self.itemmap[mid])
                # Giá trị trọng số (confidence value) cho ALS
                user_items_values.append(float(1.0 + self.alpha * 5.0))

        if not user_items_indices:
            return []

        # Tạo ma trận thưa thớt 1 hàng (1 user)
        # Cần ép kiểu .astype('float32') để tránh lỗi ambiguous types
        virtual_user_items = csr_matrix(
            (np.array(user_items_values, dtype='float32'), 
             (np.zeros(len(user_items_indices), dtype='int32'), 
              np.array(user_items_indices, dtype='int32'))),
            shape=(1, len(self.itemmap))
        )

        # Gọi hàm recommend của implicit
        # userid=0 vì chúng ta chỉ truyền vào 1 user duy nhất trong virtual_user_items
        rec = self.model.recommend(
            userid=0, 
            user_items=virtual_user_items, 
            N=top_k, 
            filter_already_liked_items=True,
